...
public sealed class GetServiceByIdQueryHandler(IServiceRepository repository) 
    : BaseResponseHandler, IRequestHandler<GetServiceByIdQuery, BaseResponse<ServiceDTO>> 
{
    public async Task<BaseResponse<ServiceDTO>> Handle(GetServiceByIdQuery request, CancellationToken ct) 
    {
        var result = await repository.FindOneAsync(x => x.Id == request.Id, (x) => new ServiceDTO { Id = x.Id, Name = x.Name });
