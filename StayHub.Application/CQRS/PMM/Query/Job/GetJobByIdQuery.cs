...
public sealed class GetJobByIdQueryHandler(IJobRepository repository) 
    : BaseResponseHandler, IRequestHandler<GetJobByIdQuery, BaseResponse<JobDTO>> 
{
    public async Task<BaseResponse<JobDTO>> Handle(GetJobByIdQuery request, CancellationToken ct) 
    {
        var result = await repository.FindOneAsync(x => x.Id == request.Id, (x) => new JobDTO { Id = x.Id, Name = x.Name });
