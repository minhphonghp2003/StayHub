...
public sealed class GetAssetByIdQueryHandler(IAssetRepository repository) : BaseResponseHandler, IRequestHandler<GetAssetByIdQuery, BaseResponse<AssetDTO>> 
{
    public async Task<BaseResponse<AssetDTO>> Handle(GetAssetByIdQuery request, CancellationToken ct) 
    {
        var result = await repository.FindOneAsync(x => x.Id == request.Id, (x) => new AssetDTO 
        { 
            Id = x.Id, 
            Name = x.Name,
            Quantity = x.Quantity,
