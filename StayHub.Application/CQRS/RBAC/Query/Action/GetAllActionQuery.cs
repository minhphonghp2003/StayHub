...
namespace StayHub.Application.CQRS.RBAC.Query.Action
{
    ...
    public sealed class GetAllActionQueryHandler(IActionRepository actionRepository, IConfiguration configuration,IHttpContextAccessor httpContextAccessor) : BaseResponseHandler, IRequestHandler<GetAllActionQuery, Response<ActionDTO>>
    {
        public async Task<Response<ActionDTO>> Handle(GetAllActionQuery request, CancellationToken cancellationToken)
        {
            Console.WriteLine($"Received GetAllActionQuery with pageNumber: {httpContextAccessor.HttpContext.GetUserId()}");
