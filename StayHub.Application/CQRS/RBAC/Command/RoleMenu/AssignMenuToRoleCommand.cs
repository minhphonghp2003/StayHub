...
namespace StayHub.Application.CQRS.RBAC.Command.RoleMenu
{
    ...
    public sealed class AssignMenuToRoleCommandHandler(IRoleActionRepository roleActionRepository,IMenuActionRepository menuActionRepository) : BaseResponseHandler, IRequestHandler<AssignMenuToRoleCommand, Response<List<int>>>
    {
        public async Task<Response<List<int>>> Handle(AssignMenuToRoleCommand request, CancellationToken cancellationToken)
        {

            var actions = (await menuActionRepository.GetManyAsync(filter:e=>request.menuIds.Contains(e.MenuId),selector:(e,i)=>e.ActionId)).ToList();
